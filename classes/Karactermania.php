...
class Karactermania
{
    ...
    public static function setKaractermaniaFTP($id_order) {
        ...
        $sql_info_ftp = "SELECT id_pedidos_karactermania, 53 AS id_supplier, id_product, id_product_attribute, product_name, referencia_prestashop, referencia_karactermania AS referencia_proveedor, unidades
        FROM lafrips_pedidos_karactermania
        WHERE ftp = 0
        AND prepedido = 0
        AND error = 0
        ...
        if ($info_ftp = Db::getInstance()->executeS($sql_info_ftp)) {
            ...
            $info['{archivo_expediciones}'] = $error_archivo.'Pedido '.$id_order.' a Karactermanía '.date("Y-m-d H:i:s");
            ...
            @Mail::Send(
                ...
                Mail::l($error_archivo.'Pedido '.$id_order.' realizado a Karactermanía '.date("Y-m-d H:i:s"), 1),
                $info,
