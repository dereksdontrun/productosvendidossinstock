    ...
    public static function checkCambioEsperandoProductos($id_order, $id_customer_thread) {
        ...
        if (!Db::getInstance()->ExecuteS($sql_otros_productos_pedido)) { 
            ...
            if ($order->current_state == Configuration::get(PS_OS_OUTOFSTOCK_PAID)){
                ...
                $data = 'current_state: '.$order->current_state;
                $order->current_state = $id_esperando_productos;
                $order->update();
                $data .= '<br>current_state: '.$order->current_state;
                $path = _PS_ROOT_DIR_."/proveedores/karactermania/pedidos/kk_order_current_state.txt";
                file_put_contents($path, $data);

                // $order->setCurrentState($id_esperando_productos, 44);
				...
				$id_carrier_orders ,
                $id_carrier_orders ,
                'A Esperando Productos - Pedido Venta Sin Stock - Automático',
				NOW())";
                ...
                if ($id_customer_thread) {
                    ...
                    $mensaje_pedido_sin_stock_estado = 'Pedido Venta Sin Stock cambiado a Esperando Productos                     
